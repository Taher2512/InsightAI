...
```bash
cd ../.. && bun install && cd apps/backend && bun run build && npx prisma generate
```

**Alternative (Using Turbo):**
```bash
cd ../.. && bun install && bunx turbo run build --filter=backend && cd apps/backend && npx prisma generate
```

**What it does:**
1. Navigate to monorepo root (`cd ../..`)
2. Install all dependencies including `db` package (`bun install`)
3. Return to backend folder (`cd apps/backend`)
4. Compile TypeScript to JavaScript (`bun run build`)
